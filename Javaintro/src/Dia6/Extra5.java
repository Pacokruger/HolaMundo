/*
Una obra social tiene tres clases de socios:
Los socios tipo ‘A’ abonan una cuota mayor, pero tienen un 50% de descuento 
en todos los tipos de tratamientos.
Los socios tipo ‘B’ abonan una cuota moderada y tienen un 35% de descuento
para los mismos tratamientos que los socios del tipo A.
Los socios que menos aportan, los de tipo ‘C’, no reciben descuentos 
sobre dichos tratamientos.
Solicite una letra (carácter) que representa la clase de un socio, y luego un
valor real que represente el costo del tratamiento (previo al descuento)
y determine el importe en efectivo a pagar por dicho socio.
 */
package Dia6;

import java.util.Scanner;

/**
 *
 * @author Paola
 */
public class Extra5 {

    /**
     * @param args the command line arguments
     */
    public static void main(String[] args) {
        Scanner leer = new Scanner(System.in);
        
        double costo=1000;
        System.out.println("Ungrese a q clase de socio pertenece");
         System.out.println("Socio A / B / C");
         String socio=leer.next();
         
         switch (socio) {
            case "A":
                System.out.println("el traatamiento cuesta: " +costo );
                costo=costo*0.5;
                System.out.println("Ud debe abonar: " +costo);
                break;
           
                 case "B":
                System.out.println("el traatamiento cuesta: " +costo );
                costo=costo*0.65;
                System.out.println("Ud debe abonar: " +costo);
                break;
                
                 case "C":
                System.out.println("el traatamiento cuesta: " +costo );
                
                break;
            default:
                System.out.println("opcion incorrecta");
        }
         
        
    }
    
}
